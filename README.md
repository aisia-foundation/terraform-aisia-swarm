...
> **v6.14.1** · code **v6.14.2** (worktree candidat non tagué), registry/images LIVE encore **v6.14.1** — module cœur — déployer AISIA sur Docker Swarm existant
...
| `image_tag` | string | `v6.14.2` | Tag d'image (manifest multi-arch requis) |
...
| `image_tag` | `string` | `"v6.14.2"` | Tag d'image AISIA à déployer (ex. v6.14.2). Doit être un manifest multi-arch (arm64 + amd64). |
